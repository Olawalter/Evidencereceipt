...
| What validators compare | The source status, the HTTP status, truncation, where text addressed to the verifier appears, and - for a STABLE source - the byte count, the normalised content digest, the raw hash, the title and the content type; then the final result, support level, reason, provenance, the required components when the outcome rests on all of them (with evidence found), the contradicted components when a contradiction decides, and the freshness outcome when it decides. |
...
| `components`, `components_decisive` | each claim component's state (`EXPLICIT`, `IMPLIED`, `ABSENT`, `CONTRADICTED`, `UNCLEAR`), its quotes and whether validators `compared` it - stored when compared (optional readings marked uncompared), empty when an earlier reason decided the outcome |
